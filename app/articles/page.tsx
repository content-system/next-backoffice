...
import { buildFilter, buildSortSearch, formatDateTime, getOffset, read, removeLimit, removePage, write } from "web-one"
...
export default async function News({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    ...
    const fields = ["id", "title", "slug", "publishedAt", "status"]
    ...
    return (
      <div>
        <div className="main-body">
          <Form id="articlesForm" name="articlesForm" className="form" noValidate={true} action="/articles">
            <header className="page-header">
              <h2>{resource.news}</h2>

              {canWrite && <Link href="/articles/new" className="btn-new" />}
            </header>
            ...
            <table className="table">
              <thead>
                <tr>
                  ...
                  <th data-field="slug">
                    <SortLink id="slugSort" href={sort.slug.url} type={sort.slug.type} text="Slug" />
                  </th>
                  <th data-field="publishedAt">
                    <SortLink id="publishedAtSort" href={sort.publishedAt.url} type={sort.publishedAt.type} text={resource.published_at} />
                  </th>
